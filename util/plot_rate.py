...
maxrate = 0
=======
>>>>>>> 9cfbdd32bd51a034168cf6b0b4e609f9fcd55691
...
for f in args.files:
    ...
    for row in data:
        ...
        if ifname not in ['eth0', 'lo']:
            ...
            try:
<<<<<<< HEAD
                newrate = float(row[column]) * 8.0 / (1 << 20)
                maxrate = max(maxrate, newrate)
                rate[ifname].append(newrate)
=======
...
plt.legend()
<<<<<<< HEAD
plt.ylim((int(args.miny), int(120)))
=======
